...
def forward_propagation(w1, w2, w3, a0):
    z1 = np.matmul(w1,a0)
    a1 = ReLU(z1)
    z2 = np.matmul(w2,a1)
    a2 = ReLU(z2)
...
def update_parameters(w1, w2, w3, dC_dw1, dC_dw2, dC_dw3, alpha):
    w1 -= alpha * dC_dw1
    w2 -= alpha * dC_dw2
    w3 -= alpha * dC_dw3
    return w1, w2, w3
...
def output(a0, w1, w2, w3):
    _,_,_,_, _, a3 = forward_propagation(w1, w2, w3, a0)
    return a3
...
def gradient_descent(a0, y, alpha, iterations):
    w1, w2, w3 = init_parameters()
    start = time.perf_counter() 
    for i in range(iterations):
        z1, a1, z2, a2, z3, a3 = forward_propagation(w1, w2, w3, a0)
        dw1, dw2, dw3 = backward_propagation(z1, a1, z2, a2, a3, w2, w3, a0, y)
        w1, w2, w3 = update_parameters(w1, w2, w3, dw1, dw2, dw3, alpha)
...
w1,w2, w3 = gradient_descent(train_images, one_hot(train_labels), 0.1,2000)    
plt.plot(loss_history)
plt.title('Loss during training, no biases')
plt.xlabel('Iteration')
plt.ylabel('Loss')
plt.savefig('lossnb.png')
plt.clf()
plt.plot(accuracy_history)
plt.title('Accuracy during training, no biases')
plt.xlabel('Iteration')
plt.ylabel('Accuracy')
plt.savefig('Accuracynb.png')
plt.clf()
plt.plot(time_history,loss_history)
plt.title('Loss during training, no biases')
plt.xlabel('Seconds')
plt.ylabel('Loss')
plt.savefig('Losstimenb.png')
plt.clf()
plt.plot(time_history,accuracy_history)
plt.title('Accuracy during training, no biases')
plt.xlabel('Seconds')
plt.ylabel('Accuracy')
plt.savefig('Accuracytimenb.png')
...
unseen = predict(output(test_images,w1, w2, w3))
with open("Results.txt","w") as f:
    f.write("No biases " + str(accuracy(unseen, test_labels)))
