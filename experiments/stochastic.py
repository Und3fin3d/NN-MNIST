...
def batch_maker(x,y):
    ...
    ybatch = []
    for i in range(n):
        ybatch.append(y[:,i:i+1])
        print(y[:,i:i+1].shape)
        batches.append(x[:,i:i+1])
    #note remainder is scrapped as we dont want batch of 1 example
    return batches, ybatch
def gradient_descent(a0, y, alpha, iterations):
    ...
    for i in range(iterations):
        for j in range(n):
            im = a0[:,j:j+1]
            z1, a1, z2, a2, z3, a3 = forward_propagation(w1, b1, w2, b2, w3, b3, im)
            dw1, db1, dw2, db2, dw3, db3 = backward_propagation(z1, a1, z2, a2, a3, w2, w3, im, y[:,j:j+1])
        ...
        if i % 500:
            ...
            plt.title('Loss during training, stochastic')
            plt.xlabel('Iteration')
            plt.ylabel('Loss')
            plt.savefig('losssto.png')
            plt.clf()
            plt.plot(accuracy_history)
            plt.title('Accuracy during training, stochastic')
            plt.xlabel('Iteration')
            plt.ylabel('Accuracy')
            plt.savefig('Accuracysto.png')
            plt.clf()
            plt.plot(time_history,loss_history)
            plt.title('Loss during training, stochastic')
            plt.xlabel('Seconds')
            plt.ylabel('Loss')
            plt.savefig('Losstimesto.png')
            plt.clf()
            plt.plot(time_history,accuracy_history)
            plt.title('Accuracy during training, stochastic')
            plt.xlabel('Seconds')
            plt.ylabel('Accuracy')
            plt.savefig('Accuracytimesto.png')
            unseen = predict(output(test_images,w1, b1, w2, b2, w3, b3))
            with open("Results.txt","w") as f:
                f.write("Stochastic " + str(accuracy(unseen, test_labels)))
    return w1, b1, w2, b2, w3, b3
