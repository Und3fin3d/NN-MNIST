...
def batch_maker(x,y,batch_size):
    ...
    for i in range(n//batch_size):
        index = i*batch_size
        ybatch.append(y[:,index:index+batch_size])
        batches.append(x[:,index:index+batch_size])
...
def gradient_descent(x, y, alpha, iterations,batch_size):
    ...
    batches,ybatch =batch_maker(x,y,batch_size)
    start = time.perf_counter() 
    for i in range(iterations):
        for a0,yb in zip(batches,ybatch):
            z1, a1, z2, a2, z3, a3 = forward_propagation(w1, b1, w2, b2, w3, b3, a0)
            dw1, db1, dw2, db2, dw3, db3 = backward_propagation(z1, a1, z2, a2, a3, w2, w3, a0, yb)
